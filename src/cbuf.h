/*****************************************************************************
 *  $LSDId: cbuf.h,v 1.9 2002/11/07 19:50:37 dun Exp $
 *  $Id$
 *****************************************************************************
...
int cbuf_get_line (cbuf_t src, char *dstbuf, int len);
/*
 *  Reads a line of data from the [src] cbuf into [dstbuf].  Reading stops
 *    after a newline or NUL which is also written into [dstbuf].  The buffer
 *    will be NUL-terminated and contain at most ([len] - 1) characters.
 *    If [len] is 0, the line will be discarded from the cbuf and nothing
 *    will be written into [dstbuf].
 *  Returns the strlen of the line on success; truncation occurred if >= [len],
 *    in which case the excess line data is discarded.  Returns 0 if a newline
 *    is not found, in which case no data is consumed.  Returns -1 on error
 *    (with errno set).
 */

int cbuf_peek_line (cbuf_t src, char *dstbuf, int len);
/*
 *  Reads a line of data from the [src] cbuf into [dstbuf], but does not
 *    consume the data read from the cbuf.  Reading stops after a newline
 *    or NUL which is also written into [dstbuf].  The buffer will be
 *    NUL-terminated and contain at most ([len] - 1) characters.
 *  The "peek" can be committed to the cbuf via a call to cbuf_drop(),
 *    but the peek+drop operation is not atomic.
 *  Returns the strlen of the line on success; truncation occurred if >= [len].
 *    Returns 0 if a newline is not found, or -1 on error (with errno set).
 */

int cbuf_put_line (cbuf_t dst, char *srcbuf, int *ndropped);
/*
