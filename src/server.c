 ...
 */
Action *
process_input(Protocol *client_prot, Cluster *cluster, 
	      Client *c)
{
	Action *act;
	String *expect;
...
match_Client_template(Cluster *cluster, Action *act, regex_t *re, String *expect)
{
	...
	if ( pmatch[0].rm_so < 0 ) return FALSE;
 ...
 */ 
void
client_reply(Cluster *cluster, Action *act)
{
...
dump_Client_Status(Client_Status status)
{
	...
	else if(status == CLI_READING) fprintf(stderr, "CLI_READING\n");
	else if(status == CLI_WRITING) fprintf(stderr, "CLI_WRITING\n");
	else if(status == CLI_DONE) fprintf(stderr, "CLI_DONE\n");
