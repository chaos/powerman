...
static int
_ptys_open(int fdm, char *pts_name)
{
	struct group	*grptr;
	int				gid, fds;
		 ...
		 const struct winsize *slave_winsize)
{
	...
	if ( (pid = fork()) < 0)
		...
		if ( (fds = _ptys_open(fdm, pts_name)) < 0)
			err(FALSE, "can't open slave pty");
		...
		if (ioctl(fds, TIOCSCTTY, (char *) 0) < 0)
			err(FALSE, "TIOCSCTTY error");
#endif
				/* set slave's termios and window size */
		if (slave_termios != NULL) {
			if (tcsetattr(fds, TCSANOW, slave_termios) < 0)
				err(FALSE, "tcsetattr error on slave pty");
		}
		...
		if (dup2(fds, STDIN_FILENO) != STDIN_FILENO)
			err(FALSE, "dup2 error to stdin");
		if (dup2(fds, STDOUT_FILENO) != STDOUT_FILENO)
			err(FALSE, "dup2 error to stdout");
		if (dup2(fds, STDERR_FILENO) != STDERR_FILENO)
			err(FALSE, "dup2 error to stderr");
