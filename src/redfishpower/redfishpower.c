...
static int status_polling_interval_set = 0;
...
static void send_status_poll(struct powermsg *pm)
{
     ...
     *
     * The exception is if the user configured a specific delay to use.
     *
     ...
     */
    if (!status_polling_interval_set) {
        if (pm->poll_count < 4)
            poll_delay = 1 * USEC_IN_SEC;
        else if (pm->poll_count < 6)
            poll_delay = 2 * USEC_IN_SEC;
        else
            poll_delay = 4 * USEC_IN_SEC;
    }
    else
