...
namespace eval quack_power {
	...
	# There are some real file locking and concurrency issues here
	# about when one is trying to do a "power on/off" and a regularly 
	# scheduled "check" occurs.  the above "power_lock" only addresses
	# it in a minimal fashion.  there is still a race condition.
	variable power_lock
	
	if {[info exists quack_lock]} {
	    incr quack_lock 
	} else {
	    set quack_lock 1
	}
	set proc "::powerlib::quack_power::power"
	if {$::app::debug == 1} {
	    puts "Entering $proc"
	}
	...
	if { $command == "reset" } {
	    set code "T"
	    ::powerlib::quack_power::power "off" $node_list
	    if {$app::tkdanger} {
		set ::powerlib::quack_power::delay_list $node_list
		after 3000 {
		    ::powerlib::quack_power::power "on" \
			    $::powerlib::quack_power::delay_list
		}
	    } else {
		after 3000 
		::powerlib::quack_power::power "on" $node_list
	    }
	    return
	}
	if { $code == "" } {
