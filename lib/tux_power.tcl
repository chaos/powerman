...
namespace eval tux_power {}

proc tux_power::power {command node_list} {
    set code ""
    if { $command == "on" } {
        set code "1"
    }
    ...
    if { $command == "reset" } {
        set code "T"
	tux_power::power "off" $node_list
	if {$app::tkdanger} {
	    set ::tux_power::delay_list $node_list
	    after 3000 {
		tux_power::power "on" $::tux_power::delay_list
	    }
	} else {
	    after 3000 
	    tux_power::power "on" $node_list
	}
	return
    }
    if { $code == "" } {
