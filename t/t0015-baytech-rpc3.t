...
test_description='Check Baytech RPC-3 (not NC) device script'
...
test_expect_success 'create powerman.conf with one rpc3 device (8 plugs)' '
	cat >powerman.conf <<-EOT
	listen "$testaddr"
	include "$rpc3dev"
	device "b0" "baytech" "$baytech -p rpc3 |&"
	node "t[0-7]" "b0"
	EOT
'
...
test_expect_success 'create powerman.conf with 16 rpc3 devices (128 plugs)' '
	...
	include "$rpc3dev"
	device "b0" "baytech" "$baytech -p rpc3 |&"
	device "b1" "baytech" "$baytech -p rpc3 |&"
	device "b2" "baytech" "$baytech -p rpc3 |&"
	device "b3" "baytech" "$baytech -p rpc3 |&"
	device "b4" "baytech" "$baytech -p rpc3 |&"
	device "b5" "baytech" "$baytech -p rpc3 |&"
	device "b6" "baytech" "$baytech -p rpc3 |&"
	device "b7" "baytech" "$baytech -p rpc3 |&"
	device "b8" "baytech" "$baytech -p rpc3 |&"
	device "b9" "baytech" "$baytech -p rpc3 |&"
	device "b10" "baytech" "$baytech -p rpc3 |&"
	device "b11" "baytech" "$baytech -p rpc3 |&"
	device "b12" "baytech" "$baytech -p rpc3 |&"
	device "b13" "baytech" "$baytech -p rpc3 |&"
	device "b14" "baytech" "$baytech -p rpc3 |&"
	device "b15" "baytech" "$baytech -p rpc3 |&"
	node "t[0-7]" "b0"
	node "t[8-15]" "b1"
	node "t[16-23]" "b2"
	node "t[24-31]" "b3"
